...
class perfect_cuckoo_map {
 public:
  ...
  const_iterator end() const { return make_hollow_end(&values_, &present_, values_.end()); }
 ...
 private:
  ...
  bool update_bucket( 
    ...
    vector<bool>* present,
    vector<rank_select_index>* index,
    uint32_t b);
  ...
  typedef vector<rank_select_index> index_type; 
  index_type index_; 
...
PC_MAP_METHOD_DECL(bool_type, pack)(bool minimal) {
  ...
  vector<rank_select_index> new_index(new_values.size()/8);
...
PC_MAP_METHOD_DECL(iterator, find)(const key_type& k) {
  ...
  cerr << "Key " << k << " present " << present_[pos] << " equal " << equal_(k, vit->first) << endl;
  if (!present_[pos] || !equal_(k, vit->first)) return end();
  return make_hollow(&values_, &present_, vit);
}
